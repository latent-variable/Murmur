...
Murmur is a local-first utility. This document says exactly what it does, what
...
- No network connection after the one-time model download (~340 MB from a public
  GitHub release). You can verify with Little Snitch / `nettop` — idle Murmur
...
Murmur's whole job: take the text you select and read it aloud. macOS protects
...
2. **Simulating ⌘C for the clipboard fallback.** When an app doesn't expose its
   selection over the Accessibility API, Murmur falls back to copying: it posts
...
computer") because that's the single switch macOS offers — but what Murmur
actually does with it is narrow and visible in
[`TextCapture.swift`](../app/Sources/Murmur/TextCapture.swift).
...
You can use Murmur without granting anything: set **Read source → Clipboard**.
Then you copy text yourself (⌘C) and press the shortcut; Murmur reads the
...
matching the new binary. Symptom: the toggle is on yet Murmur still asks.

Fix it cleanly:

1. System Settings ▸ Privacy & Security ▸ Accessibility.
2. Select **Murmur**, click **−** to remove it.
3. Relaunch Murmur and grant again (fresh entry, bound to the current binary).
...
/Applications/Murmur.app/Contents/MacOS/Murmur --diag

# runtime log of every capture attempt:
cat ~/Library/Application\ Support/Murmur/murmur.log
```
