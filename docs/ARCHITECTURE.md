...
| macOS Services provider ("Read with Murmur") | `MurmurApp.swift` (`ServiceProvider`) + `Resources/Info.plist` (`NSServices`) |
...
The **Services menu** ("Read with Murmur") is a second entry point: macOS hands the selected text straight to `ServiceProvider`, which calls `AppState.readAloud(_:)` — skipping capture, joining the pipeline at preprocess.
