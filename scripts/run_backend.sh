#!/usr/bin/env bash
# Launch the Murmur Kokoro backend. Used by the app and for manual dev runs.
...
VENV="${MURMUR_VENV:-$SUPPORT/venv}"
PORT="${MURMUR_PORT:-8765}"
MODELS_DIR="${MURMUR_MODELS_DIR:-$SUPPORT/models}"
mkdir -p "$SUPPORT"

if [ ! -x "$VENV/bin/python" ]; then
  echo "[murmur] creating venv..."
