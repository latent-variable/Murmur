...
#
# The tap repo is expected at ../homebrew-tap (override with MURMUR_TAP_DIR).
set -euo pipefail
...
TAP_DIR="${MURMUR_TAP_DIR:-$ROOT/../homebrew-tap}"
CASK="$TAP_DIR/Casks/murmur.rb"
TAG="v$VERSION"
...
[ -f "$CASK" ] || { echo "cask not found at $CASK (set MURMUR_TAP_DIR)"; exit 1; }
...
gh release create "$TAG" "$DMG" --title "Murmur $VERSION" "${NOTES_ARGS[@]}"

say "bumping Homebrew cask"
git -C "$TAP_DIR" commit -aqm "Murmur $VERSION"
